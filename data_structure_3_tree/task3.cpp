    ...
    while (op) {
        ...
        switch (op) {
            case 1: { // 创建二叉树
                ...
                printf("请依次输入%d个结点的key和others（key为-1表示空结点）：\n", n);
                for (int i = 0; i < n; ++i) {
                    printf("第%d个结点 key: ", i+1);
                    scanf("%d", &def[i].key);
                ...
                if (CreateBiTree(T, def) == OK) printf("二叉树创建成功！\n");
                else printf("二叉树创建失败（有重复key或空间不足）！\n");
                free(def);
                getchar();
                break;
            }
            ...
            case 9: { // 插入结点
                int e, LR;
                TElemType val;
                printf("请输入父结点关键字（-1表示插入为新根）：");
                scanf("%d", &e);
                printf("请输入插入方向（0左，1右，-1新根）：");
                scanf("%d", &LR);
                printf("请输入新结点 key: ");
                scanf("%d", &val.key);
            ...
            case 10: { // 删除结点
                ...
                printf("请输入要删除的结点关键字：");
                scanf("%d", &key);
                printf("未实现DeleteNode函数，请在my.h中补充实现！\n");
                getchar();
                break;
            }
            case 11: { // 前序遍历
